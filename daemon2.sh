...
find "$PATH_NOVEDADES" -type f -not -name "Entrega_[0-9][1-9].txt"|
while read file
do
 	if [ -f "$file"] then
	...
	do
	   ...
	   cant_registros_trailer=0
		...
		then
			...
			los "El trailer de " $f "es incorrecto los datos son distintos, sera rechazado"
			mv $f $PATH_RECHAZADOS
		fi
	done
...
#para iniciar esto ya hemos validado los archivos generando un archivo aceptados correcto.
	...
	do
	  ...
	  do
		...
		#si tenemos todo OK, genero o agrego registroas a los archivos
